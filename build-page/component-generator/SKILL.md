...
11. **No overflow at 320px** — see `rules/no-overflow-320.md`: a bare `w-[Npx]` on a mobile-first element must never exceed 320px minus the page's horizontal padding, or it overflows the QA floor viewport. Pair it with `w-full max-w-[Npx]` (or a fluid width token), not a bare fixed width — unless the value is genuinely small (icons, avatars, badges).
