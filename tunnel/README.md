...
**Want the same public link every time, instead of a random one?** The first time you run
`tunnel`/`./tunnel/run` in a project, it auto-creates a `config.tunnel` file at the project root
(never committed). If you have a free [ngrok static domain](https://dashboard.ngrok.com/domains),
just put it in there:
```bash
NGROK_DOMAIN="your-name.ngrok-free.app"
```
and you'll get that same link every time. Leave it empty (or don't have one) and it just uses a
random cloudflared link, like always — nothing else changes.
