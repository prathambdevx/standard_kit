...
| **`optional/`** | Patterns with a real tradeoff to weigh, so never copied in by default (vs `extras/`, which is low-risk nice-to-haves). Currently list virtualization for long-list scroll jank — buys back per-item mounted cost, costs you server-HTML presence and transient subtree state. Each doc leads with the cheaper wins that usually beat it |
