...
| **`ios-safari-fixes/`** | 8 recurring iOS-Safari-and-mobile-Chrome rendering bugs (SVG clipping, `w-auto` cropping, input auto-zoom, `dvh` viewport jump, scroll-lock, safe-area double-stacking, SVG repaint shimmer / position:fixed detachment, address-bar-aware full-screen overlays) — a skill doc with real before/after fixes + a static validator that catches 4 of them mechanically |
