...
| **`optional/`** | Patterns with a real tradeoff to weigh, so never copied in by default (vs `extras/`, which is low-risk nice-to-haves). Currently `virtualization/` — the two scroll-jank tools with the CPU-vs-GPU model that decides between them: `useVirtualization` (CPU: per-item mounted cost — engines, observers, DOM) and `useRailWindow` (GPU: horizontal scrollers rasterise at full scroll width, ~15 MB per 12-card rail; window to 6, expand on the rail's first scroll, collapse off-screen). Includes the full device-measured case study. Each doc leads with the cheaper wins that usually beat it |
